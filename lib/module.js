/**
 * @file willie-core - The Core module
 */
...
Module.prototype.webCommand = function(argv, callback) {
  ...
  webServer.start(function(err) {
    ...
    function _startModule(index, callback) {
      ...
      var module = that.modules[index].Module;
      var config = module.moduleConfig;
      return webServer.startModule(module, function(err, webApp) {
        if (err) log.warn(new Exception({module:config.name}, "Failed to start module web service", err));
        if (webApp) webApps.push(webApp);
        if (config.name === 'core') coreWebApp = webApp;
